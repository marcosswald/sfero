...
void setup() {
  ...
  for (int n = 0; n < NR_PWM_DRIVERS; n++) {
    pwm[n].begin();
    pwm[n].setOscillatorFrequency(27000000);
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    switch (opcode) {
      ...
      // 0x00 - Reset all pwm channels
      case 0x00:
        {
          Serial.println("[0x00] Reset");
          for (int n = 0; n < NR_PWM_DRIVERS; n++) {
            for (int i = 0; i < 8; i++) {
              pwm[n].setPWM(i, 0, 0);
              delay(100);
      ...
      // 0x01 - Set servo pwm and release: {addr, servo, pulse}
      case 0x01:
        {
          int tokens = 3;
          int comarray[tokens];
      ...
      // 0x02 - Set servo pwm and keep position: {addr, servo, pulse}
      case 0x02:
        {
          int tokens = 3;
          int comarray[tokens];
